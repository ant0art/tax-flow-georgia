...
8. Заполните **Authorized redirect URIs**:
   ...
   ```
   > **Примечание:** Redirect URI необходим для авторизации на мобильных устройствах.
   > На мобильных приложение использует redirect-flow вместо popup.
   > URI должен точно совпадать с URL деплоя (включая завершающий `/`).
...
| **Мобильная авторизация** | На мобильных устройствах OAuth использует redirect-flow вместо popup для лучшего UX. Токен перехватывается до монтирования приложения |
| **Истечение сессии** | При истечении токена (~60 мин) приложение автоматически выполняет logout и перенаправляет на страницу входа |
...
| **Мобильная сессия** | На мобильных фоновое обновление токена не поддерживается. Сессия истекает через ~60 минут, требуется повторная авторизация |
